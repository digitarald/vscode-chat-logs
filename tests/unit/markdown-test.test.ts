...
describe('Markdown Log Parsing', () => {
  it('should parse the test markdown log correctly', () => {
    ...
    expect(toolCalls.some((tc) => tc.type === 'tool_call' && tc.toolCall.type === 'read')).toBe(
      true
    );

    // Should have "Navigate" tool call
    expect(toolCalls.some((tc) => tc.type === 'tool_call' && tc.toolCall.type === 'navigate')).toBe(
      true
    );

    // Should have "Click" tool call
    expect(toolCalls.some((tc) => tc.type === 'tool_call' && tc.toolCall.type === 'click')).toBe(
      true
    );
    ...
    // Code blocks now appear inline as contentSegments of type 'code_block'
    const codeBlockSegments = assistantMsg.contentSegments.filter((s) => s.type === 'code_block');
    expect(codeBlockSegments.length).toBeGreaterThan(0);
    const firstBlock = codeBlockSegments[0];
    if (firstBlock.type === 'code_block') {
      expect(firstBlock.language).toBe('typescript');
      expect(firstBlock.code).toContain('const');
    }
  });
